...
int v785_segdata(unsigned int maddr){
  int wordcnt;

  wordcnt = 0;
  vread32(maddr+V785_OUTBUFF,(int *)(data+mp));
  mp += 2;
  segmentsize += 2;
  wordcnt++;
  ...
  if((data[mp-1] & V785_TYPE_MASK_S) == V785_HEADER_BIT_S){
    while(wordcnt < 34){
      vread32(maddr+V785_OUTBUFF+wordcnt*32,(int *)(data+mp));
      mp += 2;
      segmentsize += 2;
      wordcnt++;
...
int v785_segmod(short segid,unsigned int maddr){
  int tmp,wordcnt;
...
void v785_clear(unsigned int maddr){
  short sval;
...
int v785_segdata2(unsigned int maddr){
  int wordcnt;
  int tdata;
...
int v785_dmasegdata(unsigned int maddr, int rsize){
